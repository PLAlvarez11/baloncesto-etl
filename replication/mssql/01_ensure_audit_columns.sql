...
IF COL_LENGTH('dbo.Localidades', 'FechaCrea') IS NULL
  ALTER TABLE dbo.Localidades
    ADD FechaCrea DATETIME2(3) NOT NULL
    CONSTRAINT DF_Localidades_FechaCrea DEFAULT (SYSUTCDATETIME());
IF COL_LENGTH('dbo.Localidades', 'FechaActualiza') IS NULL
  ALTER TABLE dbo.Localidades
    ADD FechaActualiza DATETIME2(3) NOT NULL
    CONSTRAINT DF_Localidades_FechaActualiza DEFAULT (SYSUTCDATETIME());
GO
IF OBJECT_ID('dbo.tr_Localidades_AuditUpdate', 'TR') IS NOT NULL
  DROP TRIGGER dbo.tr_Localidades_AuditUpdate;
GO
CREATE TRIGGER dbo.tr_Localidades_AuditUpdate
ON dbo.Localidades
AFTER UPDATE
AS
BEGIN
  SET NOCOUNT ON;
  UPDATE l
     SET FechaActualiza = SYSUTCDATETIME()
  FROM dbo.Localidades l
  JOIN inserted i ON i.id_Localidad = l.id_Localidad;
END
GO
...
GO
IF OBJECT_ID('dbo.tr_Equipos_AuditUpdate', 'TR') IS NOT NULL
  DROP TRIGGER dbo.tr_Equipos_AuditUpdate;
GO
CREATE TRIGGER dbo.tr_Equipos_AuditUpdate
ON dbo.Equipos
AFTER UPDATE
AS
BEGIN
  ...
  UPDATE e
     SET FechaActualiza = SYSUTCDATETIME()
  FROM dbo.Equipos e
  JOIN inserted i ON i.id_Equipo = e.id_Equipo;
END
GO
...
GO
IF OBJECT_ID('dbo.tr_Jugadores_AuditUpdate', 'TR') IS NOT NULL
  DROP TRIGGER dbo.tr_Jugadores_AuditUpdate;
GO
CREATE TRIGGER dbo.tr_Jugadores_AuditUpdate
ON dbo.Jugadores
AFTER UPDATE
AS
BEGIN
  ...
  JOIN inserted i ON i.id_Jugador = j.id_Jugador;
END
GO
...
GO
IF OBJECT_ID('dbo.tr_Partidos_AuditUpdate', 'TR') IS NOT NULL
  DROP TRIGGER dbo.tr_Partidos_AuditUpdate;
GO
CREATE TRIGGER dbo.tr_Partidos_AuditUpdate
ON dbo.Partidos
AFTER UPDATE
AS
BEGIN
  ...
  UPDATE p
     SET FechaActualiza = SYSUTCDATETIME()
  FROM dbo.Partidos p
  JOIN inserted i ON i.id_Partido = p.id_Partido;
END
GO

/* TABLA: Cuartos*/
IF COL_LENGTH('dbo.Cuartos', 'FechaCrea') IS NULL
  ALTER TABLE dbo.Cuartos
    ADD FechaCrea DATETIME2(3) NOT NULL
...
GO
IF OBJECT_ID('dbo.tr_Cuartos_AuditUpdate', 'TR') IS NOT NULL
  DROP TRIGGER dbo.tr_Cuartos_AuditUpdate;
GO
CREATE TRIGGER dbo.tr_Cuartos_AuditUpdate
ON dbo.Cuartos
AFTER UPDATE
AS
BEGIN
  ...
  UPDATE c
     SET FechaActualiza = SYSUTCDATETIME()
  FROM dbo.Cuartos c
  JOIN inserted i ON i.id_cuarto = c.id_cuarto AND i.id_partido = c.id_partido;
END
GO
...
GO
IF OBJECT_ID('dbo.tr_Anotacion_AuditUpdate', 'TR') IS NOT NULL
  DROP TRIGGER dbo.tr_Anotacion_AuditUpdate;
GO
CREATE TRIGGER dbo.tr_Anotacion_AuditUpdate
ON dbo.Anotacion
AFTER UPDATE
AS
BEGIN
...
GO
IF OBJECT_ID('dbo.tr_Falta_AuditUpdate', 'TR') IS NOT NULL
  DROP TRIGGER dbo.tr_Falta_AuditUpdate;
GO
CREATE TRIGGER dbo.tr_Falta_AuditUpdate
ON dbo.Falta
AFTER UPDATE
AS
BEGIN
