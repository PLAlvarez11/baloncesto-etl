...
def main():
    ...
    # Dependencias: primero dims, luego facts
            ...
            for tbl in group:
                try:
                    ...
                    if tbl == "Localidades":
                        ...
                        # necesitamos nombre de equipo; lo derivamos vía join EF en ETL real,
                        # aquí asumimos que el cambio trae id_Equipo y luego resolvemos nombre con consulta al DW.
                        for ch in changes:
                            if ch.get("__$operation") in (2,4):
                                # resolve equipo_nombre por nk_equipo_id si estuviera:
                                equipo_nombre = None
                                with pg.connect() as cx:
                                    rs = cx.execute(text("""
                                      SELECT nombre FROM dw.dim_equipo WHERE nk_equipo_id = :nk
                                    """), {"nk": ch.get("id_Equipo")}).first()
                                    equipo_nombre = rs[0] if rs else None
                                upsert_jugador(pg, ch, equipo_nombre)
                                rows_applied += 1

                    elif tbl == "Partidos":
                        for ch in changes:
                            if ch.get("__$operation") in (2,4):
                                # Resolver nombres local/visita/localidad por NKs si vienen en cambio
                        ...
                        # apoyo a hechos (no se carga como dim), nada que aplicar directo al DW
                        pass
                        ...
                        for ch in changes:
                            if ch.get("__$operation") in (2,4):
                                ...
                                insert_fact_anotacion(pg, payload)
                                rows_applied += 1
