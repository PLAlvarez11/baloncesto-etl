...
DO UPDATE SET nombre=EXCLUDED.nombre, ciudad=EXCLUDED.ciudad, activo=TRUE;
"""
...
DO UPDATE SET nombre=EXCLUDED.nombre,apellido=EXCLUDED.apellido,numero=EXCLUDED.numero,
              edad=EXCLUDED.edad,estatura_cm=EXCLUDED.estatura_cm,
              nacionalidad=EXCLUDED.nacionalidad,equipo_actual=EXCLUDED.equipo_actual;
"""
...
DO UPDATE SET fecha_hora=EXCLUDED.fecha_hora,equipo_local=EXCLUDED.equipo_local,
              equipo_visita=EXCLUDED.equipo_visita,localidad=EXCLUDED.localidad;
"""
...
VALUES (DATE(:ts), EXTRACT(YEAR FROM :ts)::INT, EXTRACT(MONTH FROM :ts)::INT, EXTRACT(DAY FROM :ts)::INT,
        ((EXTRACT(MONTH FROM :ts)::INT -1)/3 +1)::INT,
...
WITH s AS (
  SELECT :nk_partido_id::INT nk_partido_id, :nk_equipo_id::INT nk_equipo_id,
         :nk_jugador_id::INT nk_jugador_id, :cuarto::INT cuarto,
         :puntos::INT puntos, :ts_evento::TIMESTAMP ts_evento
)
...
WITH s AS (
  SELECT :nk_partido_id::INT nk_partido_id, :nk_equipo_id::INT nk_equipo_id,
         :nk_jugador_id::INT nk_jugador_id, :cuarto::INT cuarto,
         :cantidad::INT cantidad, :ts_evento::TIMESTAMP ts_evento
)
