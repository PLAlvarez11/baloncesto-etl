...
def mssql_engine() -> Engine:
    ...
    conn=f"mssql+pyodbc://{user}:{pwd}@{host}:{port}/{db}?driver={drv.replace(' ','+')}\
...
def ensure_repl_tables(pg: Engine):
    with pg.begin() as cx:
        ...
        CREATE TABLE IF NOT EXISTS dw.metrics_replication(
          ...
          from_lsn BYTEA, to_lsn BYTEA,
          rows_read BIGINT, rows_applied BIGINT,
          status VARCHAR(20), message TEXT
        );"""))

def get_last_lsn(pg: Engine, table_name: str):
...
def upsert_last_lsn(pg: Engine, table_name: str, lsn: bytes, processed: int, note="OK"):
    with pg.begin() as cx:
        cx.execute(text("""
...
def add_metric(pg: Engine, table: str, from_lsn: bytes, to_lsn: bytes,
               rows_read:int, rows_applied:int, status:str, message:str):
    with pg.begin() as cx:
        cx.execute(text("""
        ...
        """),{"t":table,"f":from_lsn,"to":to_lsn,"rr":rows_read,"ra":rows_applied,
              "s":status,"m":message[:800]})
